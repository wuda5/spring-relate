...
//		TimeDaoInterface time = ioc.getBean(TimeDaoImplOne.class);// 报错
		...
		System.out.println("+++++ 采用 jdk 代理时 获取的bean（proxy代理对象） 是否和 目标对象 类型同:【 "+	(time instanceof TimeDaoImplOne) +" 】bean 代理对象：【"+time.getClass() );

		time.timeDaoYes(1, 9);/**1. JdkDynamicAopProxy.invoke */
		System.out.println("===========================================");
