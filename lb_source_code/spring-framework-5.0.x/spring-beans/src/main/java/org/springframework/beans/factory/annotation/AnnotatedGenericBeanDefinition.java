 ...
 *
 * --重要：这个AnnotatedGenericBeanDefinition存的就是类的基本信息
 * 相当于当时自己模拟spring的时候定义了map<name ,bean> 中的name--只不过spring用这个更加复杂对象来做
 * 把他看作一个数据结构！
 *
...
public class AnnotatedGenericBeanDefinition extends GenericBeanDefinition implements AnnotatedBeanDefinition {
	...
	/**
	 * AnnotationMetadata接口：这是存bean类的注解，实现是：new StandardAnnotationMetadata(beanClass, true);
	 *
	 *  1.其实，AnnotationMetadata  和下面的 MethodMetadata 都是继承 了AnnotatedTypeMetadata
	 *  2.只不过 AnnotationMetadata多继承了个 ClassMetadata
	* */
	private final AnnotationMetadata metadata;

	/**
	 * MethodMetadata：这是存bean的元数据：如名字，类型等等
	 *
	 */
	@Nullable
