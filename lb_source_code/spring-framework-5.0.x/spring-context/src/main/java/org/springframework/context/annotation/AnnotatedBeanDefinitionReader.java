...
public class AnnotatedBeanDefinitionReader {
	...
	public AnnotatedBeanDefinitionReader(BeanDefinitionRegistry registry) {
		/**
		 * 参数registry其实就是当前上下文AnnotationConfigApplicationContext，因为传的是this就是自己，
...
//		Assert.notNull(environment, "Environment must not be null");
		 ...
		 * 也就是这个方法里面会注册一些--用于处理注解的处理器--到registry
		 * （其实是到DefaultListableBeanFactory上面，但是由于registry 就是上下文，而DefaultListableBeanFactory
		 * 算上下文的父类的一个属性，即可以看作是作用到registry--即也是到AnnotationConfigApplicationContext ）
		 *
		 * * --最终调用注册就是在DefaultBeanFactory 中：this.beanDefinitionMap.put(beanName, beanDefinition)
		 * 	 * 将spring默认的7！！个beandef 注册put入map
		 */
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		 ...
		 * scope，lazy等等
		 *
		 * 将AnnotatedGenericBeanDefinition看作一个bean的描述器！顶层接口：BeanDefinition
		 * 因为我们调用的是注解类到这个方法，所以我们可以直接new 注解相关的BeanDefinition 的bean 描述器
		 * 即我们可以直接new AnnotatedGenericBeanDefinition
		 */
		...
		/**
		 * 得到类的--scope作用域--其实这里是对注册单个bean的时候如：ioc.register(xxx.class) 有用
		 */
		...
		/**
		 * 处理类当中的通用注解---由最上面的register(annotatedClasses)调用，
		 * 但是此方法，后面在refresh()的时候还会--再调用一遍（那时主要是包扫描scan其他bean！！！！！！,第一次是通过register方法过来的仅仅解析了appconfig类的注解信息）
		 * 分析源码可以知道他主要处理
		...
		/**
		 * 这个BeanDefinitionHolder也是一个数据结构,把他看作一个map
		 */
		...
		/**
		 * 把上述的这个数据结构注册给registry--最终操作是put 到 DefaultLisableBeanFactory中 维护的map中--2个集合
		 * 1.beanDefinitionMap 2. beanDefinitionNames
		 *
		 * registy就是AnnotatonConfigApplicationContext---因为AnnotatonConfigApplicationContext继承了它的，
		 * 并且在AnnotatonConfigApplicationContext中用自己构造初reader，--this.reader = new AnnotatedBeanDefinitionReader(this);
