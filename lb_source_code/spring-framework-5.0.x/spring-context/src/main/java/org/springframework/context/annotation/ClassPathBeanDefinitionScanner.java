...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	// --这里也是维护了spring的上下文registry也即AnnotationConfigApplicatonContext！！
	...
	/**--尼玛注意：这个方法最后操作一部非常关键：registerBeanDefinition(definitionHolder, this.registry);通过配置类文件的路径basePackages（com.lun）将扫描bean 的bd注册了！！！！，并没吧
	 * Perform a scan within the specified base packages,
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			//符合条件的并把它转成BeanDefinition类型（还没注册bd到map，这里完后，下面对这些bd进行在完整的注解解析后，在最后放入map注册的）
			...
			for (BeanDefinition candidate : candidates) {
				...
				if (candidate instanceof AnnotatedBeanDefinition) {
					...
					//当前前提是这个类必须是AnnotatedBeanDefinition类型的，说白了就是加了注解的类--注意：这里就是将前面通过扫描配置类appconfig（从其注解scan上的com.luban得到了所有满足条件的注解类的bd了，这一步就是在对这些bd在操作，解析他们各自类上的注解信息完成完整的bd?,注意：此时这些bean的bd还不完整所以还没放到单例map中，等所有bd经过此步操作后，才最终放入map完成bd的注册registerBeanDefinition(definitionHolder, this.registry);）
