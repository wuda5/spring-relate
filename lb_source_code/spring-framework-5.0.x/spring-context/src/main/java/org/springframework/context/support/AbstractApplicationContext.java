...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			/**
			 * 写错了：这里会做一步很重要的事情：缓存上下文中，为后面invokeBeanFactoryPostProcessors(beanFactory);方法调用提前准备好要扫描包的类的信息(类的注解等)
			 * */
			prepareRefresh();
			...
			try {
				...
				// Initialize other special beans in specific context subclasses.---关键了：这里才是又会再执行上面fresh的相关了。卧槽--是空方法
				onRefresh();//--暂时可以不关注

				// Check for listener beans and register them.--暂时可以不关注
				registerListeners();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 这两个Bean就是一些系统配置和系统环境信息--卧槽这几个bean都没有在bdMap中注册就直接放入可以用的singtonMap中成bean了？
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		/**刚调用到这里时候， getBeanFactoryPostProcessors()是一个都获取不到的，需要等执行到里面
		 * String[] postProcessorNames =beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);
		 * 方法后，postProcessorNames 是个类路径后通过beanFactory.getBean 生成后，再加入工厂的this.beanFactoryPostProcessors;，才会有
		 *
		 * currentRegistryProcessors.add(beanFactory.getBean(ppName, BeanDefinitionRegistryPostProcessor.class)); 中后面再有 createBean 操作
		 * 会在currentRegistryProcessors.add(beanFactory.getBean(ppName, BeanDefinitionRegistryPostProcessor.class));里
		 * 将重要的 configrurationClassPostProcessor （实现了 BeanDefinitionRegistryPostProcessor 继承了 BeanFactoryPostProcessor ）实例初始放入sintonMap*/
	...
	/**关键代码最后一行：beanFactory.preInstantiateSingletons();
